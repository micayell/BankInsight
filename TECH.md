...
- **차트 및 데이터 시각화**: `Chart.js (v4.4.9)` & `Plotly.js` - 금리 비교, 환율 변동, 금/석유 시세 현황 등을 위한 인터랙티브 그래프 및 차트 구현
...
- **외부 서버 모듈 통신**: `Requests`, `Httpx` 라이브러리를 통한 외부 금융 데이터/시세 API(한국수출입은행 환율 정보, 금감원 금융상품통합비교공시, **금융결제원 오픈뱅킹 API** 등) 서버와의 HTTP 요청
...
- **스크래핑/크롤링**: 금/석유 시세(spot price) 등의 데이터를 정기적으로 가져오기 위한 스크래퍼/Cron(커스텀 커맨드) 아키텍처 포함
...
- `apis/`: 각 도메인별(인증, 게시글, 상품 등) axios 통신 로직 모듈화 폴더 
- `components/`: Chatbot, 환율계산기, 검색, 홈 캐러셀 등 공통적으로 재사용 가능한 Vue 컴포넌트 조각들
- `router/`: 프론트엔드 페이지 라우팅 설정 파일(`index.js`) 
- `stores/`: Pinia 기반 전역 상태 관리 모듈 (회원상태, 추천목록, 환율, 지도 맵 데이터 등)
...
- `financial_products/`: 예/적금 및 **주택담보/전세자금 대출** 상품 정보 저장, 사용자 간 상품 추천/비교 데이터 처리 애플리케이션
- `kftc/`: **금융결제원(KFTC) 오픈뱅킹 연동 관리를 위한 신규 애플리케이션**
- `spot/`: 오픈 API 기반 실물(**금, 석유** 등) 시세 데이터 수집 및 가공 애플리케이션
