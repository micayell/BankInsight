    ...
    <div v-else-if="product && product.fin_prdt_cd" class="content-card p-4 p-md-5 shadow-sm">
      ...
      <section class="product-options mb-4">
        ...
        <div v-if="options.length > 0" class="list-group">
          <div v-for="opt in options" :key="`${opt.intr_rate_type_nm}-${opt.save_trm}-${opt.rsrv_type_nm}`" class="list-group-item option-item">
            ...
            <div v-if="opt.rsrv_type_nm" class="mt-1 option-meta-info"><small>적립 유형: {{ opt.rsrv_type_nm }}</small></div>
          </div>
        </div>
...
export default {
  ...
  methods: {
    fetchData(code) {
      ...
      const fetchProduct = axios.get(`${base}/savings/${code}/`, { headers }); 
      const fetchOptions = axios.get(`${base}/savings/${code}/options/`, { headers }); 
      ...
      Promise.all([fetchProduct, fetchOptions])
        ...
        .catch(err => { console.error(`적금 상품(code:${code}) 정보/옵션 실패:`, err); this.product = null; this.options = []; })
...
.option-meta-info { font-size: 0.8rem; color: #6c757d; }
